...
def pre_process(events):
	if events["status"] == "cancelled":
		if frappe.db.exists("Shared Place Booking", dict(gcalendar_sync_id=events["id"])):
			e = frappe.get_doc("Shared Place Booking", dict(gcalendar_sync_id=events["id"]))
			frappe.delete_doc("Shared Place Booking", e.name)
		return {}
